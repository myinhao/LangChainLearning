...
class BaseAgent:
    ...
    def chat_with_parse(self, user_input):
        """带解析的聊天"""
        raw_response = self.chat(user_input)
    ...
    def run_parsed_chat_loop(self):
        ...
        while True:
            ...
            parsed_result = self.chat_with_parse(user_input)
            print(f"解析结果: {parsed_result}")
            print(f"类型: {type(parsed_result).__name__}")
            print("-" * 50)