...
class ReviewerAgent(Agent):
    ...
    def __init__(self, llm):
        super().__init__(
            llm,
            "你是一个专业的内容审查员，擅长评估文章的质量、结构、语法和逻辑。"
        )
...
def main():
    ...
    content_task = f"请写一篇关于 '{topic}' 的简短文章，约200字。"
    content = writer.run(content_task)
    print("生成的内容:")
    print(content)
    
    print("\n=== Writer Agent 正在写入文件 ===")
    write_result = writer.write_file(file_path, content)
    print(write_result)
    
    print("\n=== Reviewer Agent 正在审查文件 ===")
    file_content = reviewer.read_file(file_path)
    review_task = f"请审查以下内容，提供详细的评估和改进建议：\n\n{file_content}"
    review = reviewer.run(review_task)
    print("\n审查意见:")
    print(review)
